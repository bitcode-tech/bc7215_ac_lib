...
/* ESP-IDF port note:
 * This project receives UART data from a FreeRTOS task inside Bc7215Drv,
 * not from a UART ISR, and uses the BC7215 BUSY pin as the UART CTS input
 * for TX hardware flow control. Therefore the three settings above are:
 *   TX_HW_FLOW_CONTROL = 1
 *   USE_UART_INTERRUPT = 0
 *   PROCESS_IN_DIFFERENT_THREAD = 1
 */
